...
def dsg_update_file_options(directory):
    ...
    for file in os.listdir(directory):
        if file.endswith(".png") or file.endswith(".jpg"):
            #dsg_fullpath = os.path.join(directory, dirname)
            dsg_file_options.append(file)
            #dsg_file_options.append(dsg_fullpath)
    dsg_file_combobox["values"] = dsg_file_options
    dsg_file_combobox.current(0)
...
def anlx_dsg(anlx_dir, dsg_file):
    anilox_list = []
    dsg_path = os.path.join(dsg_directory_var.get(), dsg_file)
    with Image.open(dsg_path) as img1:
        img1.load()
...
anlx_check_btn = Button(frame, text='Проверить анилоксы',
                        command= lambda:anlx_dsg(directory_var.get(), dsg_file_var.get()))
